...
def export_grant_report_markdown(
    ...
    if award:
        funder_display = (
            award.funder.name if award.funder else funder_name or "Unknown"
        )
    ...
    lines.append("---")
    lines.append(
        f"*Generated by labpubs on {date.today().isoformat()}."
        " Data from OpenAlex.*"
    )
...
def export_grant_report_json(
    ...
    return orjson.dumps(
        report, option=orjson.OPT_INDENT_2
    ).decode()
...
def export_grant_report_csv(works: list[Work]) -> str:
    ...
    for work in works:
        ...
        lines.append(
            f'"{title}",{year},"{venue}",{doi},"{authors}"'
        )
    return "\n".join(lines)
...
def export_grant_report(
    ...
    if report_format == "markdown":
        return export_grant_report_markdown(
            works, award, funder_name, include_abstract
        )
    if report_format == "json":
        return export_grant_report_json(
            works, award, funder_name
        )
    if report_format == "csv":
        return export_grant_report_csv(works)
    raise ValueError(
        f"Unsupported format: {report_format}. "
        "Supported: markdown, json, csv"
    )
